	...
	extends SimulationFrame
{
	 ...
	 * 
	 * @param level
	 *            definition for the current level
	 */
	public RoThro(Level level)
	{
		...
		p1 = new Avatar();
		p1.translate(
			level.getBallPos().x - ball.getRadius() - 1,
			level.getBallPos().y);
	...
	protected void initializeWorld()
	{
		...
		this.world
			.addContactListener(new ContactListenerAdapter<SimulationBody>() {
				@Override
				public
					void
					begin(ContactCollisionData<SimulationBody> col, Contact c)
				{
					...
					else if (body2 instanceof Avatar
						&& body1 instanceof Obstacle)
					{
	...
	protected void initializeSettings()
	{
		...
		Obstacle left = new WallObstacle(
			new Rectangle(1.0, height),
			-width / 2,
			0,
			false,
			levelNum);
		Obstacle bottom = new WallObstacle(
			new Rectangle(width, 1.0),
			0,
			-height / 2,
			false,
			levelNum);
		Obstacle top = new WallObstacle(
			new Rectangle(width, 1.0),
			0,
			height / 2,
			false,
	...
	private void addPrisJoint(RoThroJoint pris)
	{
		...
		PrismaticJoint<SimulationBody> pj = new PrismaticJoint<SimulationBody>(
			anchor,
			body,
			pris.getAnchorPnt1(),
			pris.getAxis());
	...
	private void addRevJoint(RoThroJoint rev)
	{
		...
		RevoluteJoint<SimulationBody> rj =
			new RevoluteJoint<SimulationBody>(b2, b1, rev.getAnchorPnt1());
		if (rev.getLimEn())
		{
	...
	private void addDistJoint(RoThroJoint dist)
	{
		...
		DistanceJoint<SimulationBody> spring =
			new DistanceJoint<SimulationBody>(
				anchor,
				mass,
				dist.getAnchorPnt1(),
				dist.getAnchorPnt2());
	...
	private void addPendJoint(RoThroJoint pend)
	{
		...
		Obstacle rod = new Obstacle(
			new Rectangle(0.5, rodLength),
			supptPos.x,
			rodY,
			new Color(200, 20, 20),
			true,
		...
		RevoluteJoint<SimulationBody> hinge1 =
			new RevoluteJoint<SimulationBody>(support, rod, supptPos);
		RevoluteJoint<SimulationBody> hinge2 =
			new RevoluteJoint<SimulationBody>(bob, rod, bobPos);

		RevoluteJoint<SimulationBody> pendulum =
			new RevoluteJoint<SimulationBody>(support, bob, supptPos);
		pendulum.setCollisionAllowed(false);

		if (pend.getLimEn())
		{
			pendulum
				.setLimitsEnabled(pend.getLowerLimit(), pend.getUpperLimit());
		}

		FrictionJoint<SimulationBody> f =
			new FrictionJoint<SimulationBody>(support, bob, supptPos);
		f.setMaximumForce(0);
	...
	protected void gameLoopLogic()
	{
		...
		if (Math.abs(ballCoords.x) > width / 2 + ball.getRadius()
			|| Math.abs(ballCoords.y) > height / 2 + ball.getRadius())
		{
			this.stop();
		}
	}
